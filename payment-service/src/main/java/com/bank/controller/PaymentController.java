...
public class PaymentController {
    ...
    public ResponseEntity<PaymentDto> makePayment(@RequestBody PaymentDto dto) {
        System.out.println("PaymentController - makePayment called with: " + dto);
    ...
    public ResponseEntity<List<PaymentDto>> getAllTransactions(@RequestHeader(HttpHeaders.AUTHORIZATION) String authHeader,
                                                              @RequestHeader(value = "X-User-Role", required = false) String role) {
        System.out.println("PaymentController - getAllTransactions called");
        System.out.println("PaymentController - Auth Header: " + authHeader);
        System.out.println("PaymentController - User Role: " + role);
        
        if (role == null || !(role.equals("ADMIN") || role.equals("EMPLOYEE"))) {
            System.out.println("PaymentController - Access denied. Role: " + role);
            return ResponseEntity.status(HttpStatus.FORBIDDEN).build();
        }
        
        System.out.println("PaymentController - Access granted for role: " + role);
        return ResponseEntity.ok(paymentService.getAll());
    }
    
    // Get transactions for a specific account (for customers)
    @GetMapping("/transactions/account/{accountId}")
    public ResponseEntity<List<PaymentDto>> getTransactionsByAccount(@PathVariable Long accountId,
                                                                    @RequestHeader(HttpHeaders.AUTHORIZATION) String authHeader) {
        System.out.println("PaymentController - getTransactionsByAccount called for accountId: " + accountId);
        
        if (accountId == null) {
            System.out.println("PaymentController - accountId is null");
            return ResponseEntity.badRequest().build();
        }
        
        return ResponseEntity.ok(paymentService.getTransactionsByAccount(accountId));
    }
}